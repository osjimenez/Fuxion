...
public static class LifetimeExtensions
{
	...
	{
		/// <summary>
		/// Wraps the underlying value in a <see cref="DisposableEnvelope{T}"/> that executes a synchronous action when disposed.
		/// </summary>
		...
		/// allowing the envelope to be used directly as the wrapped value without explicitly accessing the <c>Value</c> property.
		/// </para>
		/// </remarks>
		/// <example>
		/// <code>
		...
		public DisposableEnvelope<T> AsDisposable(Action<T>? actionOnDispose = null)
			=> me.Value is null ? DisposableEnvelope<T>.Disposed : new(me.Value, actionOnDispose);

		/// <summary>
		/// Wraps the underlying value in a <see cref="DisposableEnvelope{T}"/> that executes an asynchronous function when disposed.
		/// </summary>
		...
		public DisposableEnvelope<T> AsDisposableAsync(Func<T, ValueTask> functionOnDispose)
			=> me.Value is null ? DisposableEnvelope<T>.Disposed : new(me.Value, functionOnDispose);
	}
}
