...
public sealed class FilterSourceGenerator : IIncrementalGenerator
{
	...
	private void Generate(SourceProductionContext context, Candidate cand)
	{
		...
		sb.AppendLine("using Fuxion.Linq.Filter.Operations;");
		sb.AppendLine("using Fuxion.Linq.Filter.Json;");
		...
		foreach (var d in cand.Descriptors.Where(x => x.Kind == DescriptorKind.Property))
		{
			...
			if (d.IsEnum) ifaceList.Add($"IHasFlagFilterOperation<{bt}>");
			var inherit = string.Join(",\r\n\t\t", ifaceList.Distinct());
			...
			sb.AppendLine($"\t#endregion {d.Name}");
			sb.AppendLine();
		}
		...
		// IsDefined override (uses HasSomeOperationsDefined for operation nodes, HasAny() for nested filters)
		var parts = new List<string>();
		foreach (var d in cand.Descriptors.Where(d => d.Kind is DescriptorKind.Property or DescriptorKind.Navigation or DescriptorKind.CollectionScalar or DescriptorKind.NavigationCollection))
		{
			var expr = d.Kind switch
			{
				DescriptorKind.Property => $"((IFilterOperation){d.Name}).IsDefined",
				DescriptorKind.CollectionScalar => $"((IFilterOperation){d.Name}).IsDefined",
				DescriptorKind.Navigation => $"((IFilterOperation){d.Name}).IsDefined",
				DescriptorKind.NavigationCollection => $"((IFilterOperation){d.Name}).IsDefined",
				_ => "false"
			};
			parts.Add(expr);
		}
