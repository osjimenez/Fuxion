...
//public class EntityFrameworkTest(ITestOutputHelper output, DatabaseFixture database) : BaseTest<EntityFrameworkTest>(output)
//{
//	[Fact]
//	public void Test()
//	{
...
//		var res = await database.Data.GetUsers().Filter(filter).CountAsync(TestContext.Current.CancellationToken);
//		Assert.Equal(1, res);
//	}
...
//		var filterCount = await database.Data.GetUsers().Filter(filter).CountAsync(TestContext.Current.CancellationToken);
//		var linqCount = await database.Data.GetUsers().Where(u => u.Phones.Any(p => p == "+34657890123")).CountAsync(TestContext.Current.CancellationToken);
...
//		PrintVariable(filter.Predicate);
//		var filterCount = await database.Data.GetUsers().Filter(filter).CountAsync(TestContext.Current.CancellationToken);
//		var linqCount = await database.Data.GetUsers().Where(u => u.Phones.All(p => p == "+34657890123")).CountAsync(TestContext.Current.CancellationToken);
//		PrintVariable(filterCount);
//		Assert.Equal(linqCount, filterCount);
//	}
...
//		var linqCount = await database.Data.GetUsers().Where(linqPredicate).CountAsync(TestContext.Current.CancellationToken);
