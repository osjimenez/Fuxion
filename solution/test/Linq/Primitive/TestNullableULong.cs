...
public class NullableULongDao
{
	public ulong? Property { get; set; }
}

[FilterSchema(nameof(Fields))]
public partial class NullableULongFilter
{
	public static readonly IFilterDescriptor<NullableULongDao>[] Fields = FilterBuilder
		.For<NullableULongDao>()
		.Property(d => d.Property)
		.Build();
}

public class TestNullableULong
{
	IQueryable<NullableULongDao> GetQueryable(params ulong?[] values) => values.Select(v => new NullableULongDao { Property = v }).AsQueryable();

	[Fact]
	public void IsNull()
	{
		var q = GetQueryable(5, null, 7, null);
		var f = new NullableULongFilter();
