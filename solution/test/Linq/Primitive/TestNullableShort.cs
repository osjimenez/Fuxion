...
public class NullableShortDao
{
	public short? Property { get; set; }
}

[FilterSchema(nameof(Fields))]
public partial class NullableShortFilter
{
	public static readonly IFilterDescriptor<NullableShortDao>[] Fields = FilterBuilder
		.For<NullableShortDao>()
		.Property(d => d.Property)
		.Build();
}

public class TestNullableShort
{
	IQueryable<NullableShortDao> GetQueryable(params short?[] values) => values.Select(v => new NullableShortDao { Property = v }).AsQueryable();

	[Fact]
	public void IsNull()
	{
		var q = GetQueryable(5, null, 7, null);
		var f = new NullableShortFilter();
