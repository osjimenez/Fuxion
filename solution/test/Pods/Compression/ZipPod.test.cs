...
namespace Test.Pods.Compression;
...
public class ZipPodTest(ITestOutputHelper output) : BaseTest<ZipPodTest>(output)
{
	...
	public void Zip()
	{
		...
		var builder = payload.Fx.Pod.BuildPod()
			.ToPod("testPayload")
		...
		Output.WriteLine($"json:\r\n{json.Pod.Payload.ToJsonString(JsonSerializerOptions.Formatted)}");
		var utf = json.ToUtf8Bytes("utf");
		Output.WriteLine($"utf:\r\n{utf.Pod.Payload.Fx.Encoding.ToBase64String().Payload}");
		var zip = utf.ToZip("zip");
		Output.WriteLine($"zip:\r\n{zip.Pod.Payload.Fx.Encoding.ToBase64String().Payload}");
		// Same as fluent
		var bytes = payload.Fx.Pod.BuildPod().ToJsonNode("json").ToUtf8Bytes("utf").ToZip("zip").Pod.Payload;
	}
	...
	public void Unzip()
	{
		...
		var unzip = base64.Fx.Encoding.ToBytesFromBase64String().Payload!
			.Fx.Pod.BuildPod().FromZip("unzip");
		Output.WriteLine($"unzip:\r\n{unzip.Pod.Payload.Fx.Encoding.ToBase64String().Payload}");
		...
		// Same as fluent
		var payload = base64.Fx.Encoding.ToBytesFromBase64String()
			.Payload!
			.Fx.Pod.BuildPod()
			.FromZip("unzip")
			.FromUtf8Bytes("utf")
